...
//RENDERERS START
unsigned int ByteToInt(unsigned char* byte)
{
	int iValue = byte[0];
	iValue += (byte[1] << 8);
	iValue += (byte[2] << 16);
...
void ExportDetails(void)
{
	...
	int iOffset = 0;
	unsigned char* pWriteData = new unsigned char[1024 * 1024 * 8];
...
void CreateBaseline( int player, int eindex, struct entity_state_s *baseline, struct edict_s *entity, int playermodelindex, Vector* player_mins, Vector* player_maxs )
{
	...
	// render information
	baseline->rendermode	= (unsigned char)entity->v.rendermode;
	baseline->renderamt		= (unsigned char)entity->v.renderamt;
	baseline->rendercolor.r	= (unsigned char)entity->v.rendercolor.x;
	baseline->rendercolor.g	= (unsigned char)entity->v.rendercolor.y;
	baseline->rendercolor.b	= (unsigned char)entity->v.rendercolor.z;
	baseline->renderfx		= (unsigned char)entity->v.renderfx;
